...
I filtered the data frame for rows that had a country_id of BDV.
I grouped by site_id and counted the number of unique ids for each site_id.
I then arranged it in descending order by the number of unique ids to get the site_id
with the largest number of unique users in the first row of the outputted data frame.
...
I first limited the data frame to only entries that fall between the desired time frame.
I then grouped by user_id and site_id so I could count the number of times each user visited each site.
I arranged this data in descending order by the number of visits to get the users who
visited a certain site more than 10 times in the first four rows.
...
I then grouped by site_id to count the number of users whose last visit was to that site.
I arranged this in descending order by the number of visitors to get the top 3 sites in the first 3 rows.
...
If this number was 1, then that means the first site they visited was also the last site they visited.
If this number was 2, then their first site was not their last site.
Since the first metric is the number that we are interested in, I counted the number of users
...
I want a model that has a moderate training error and a moderate test error because
I want it to to fit the trend but also be generalizable enough to perform on unseen data.
Such a model is Model 6: column C ~ column A.